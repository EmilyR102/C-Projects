...
bool access_cache(cache_t *cache, unsigned long addr, enum action_t action) { //Build up implementation
  ...
  {
    c = &cache->lines[index][way];
    hit = (c->tag == tag) && (c->state == VALID);

    if (hit){
      if(action == STORE) c->dirty_f = true;
      cache->lru_way[index] = (1 + way)%cache->assoc; 
      break;
    }
  }

  if (cache->assoc == 1){ //direct mapped lru always 0
    cache->lru_way[index] = 0;
  }

  if (hit == false){
    wb = c->dirty_f;
    int ind = cache->lru_way[index]; 
    c = &cache->lines[index][ind]; // load new data into lru_way
    c->state = VALID;
    c->tag = tag;
    cache->lru_way[index] = (1 + ind) % cache->assoc; //way

    if (action == STORE){
      if (c->dirty_f == false) c->dirty_f = true;
      // else{
      //   c->dirty_f = false;
      // }
    }
