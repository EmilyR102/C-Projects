...
/*
(B_bus_to_cache) is easy to calculate: it is just the number of misses multiplied by the block size
...
void calculate_stat_rates(cache_stats_t *stats, int block_size) {
  ...
  int miss = stats->n_cpu_accesses - stats->n_hits;
  stats->B_bus_to_cache = miss*block_size;
